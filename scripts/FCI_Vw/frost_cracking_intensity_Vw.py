...
def cracking_intensity(arg):
	...
	MAT = arg[0]				# Mean annual temperature at grid cell [deg C]
	Ta = arg[1]				# half amplitude of annual temperature temperature variations [deg C]

	Hs = arg[2]				# soil depth at grid cell [m]
	Py = 365					# periodicity = no. of days in an year
	N = 201					# number of depth nodes from surface to 20 m
	Ne = N-1        				# number of depth elements
	D = 20					# investigation depth [m]	
	z = np.zeros((N), dtype = np.float64)	# array containing lower boundary of each node
	ze = np.zeros((Ne), dtype = np.float64)	# array containing depth of each node
	dz = np.zeros((Ne), dtype = np.float64)	# array containing thickness of each depth node
	z[0] = 0.0				# nodal depth (0 - 20 m with depth interval of 0.1 m)
	dz0 = 0.005				# initial depth interval [m]
	for i in range(1,N):			# calculation of thickness (dz) and depth of each depth node (ze) (fine near surface, coarse at greater depths)
		z[i] = z[i-1] + dz0
	for i in range(1,N):
		z[i] = D*pow(z[i],3.0)
	for i in range(Ne):
