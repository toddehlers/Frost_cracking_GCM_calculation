...
path1 = "/esd/esd01/docs/hsharma/data_small/FCI_data/FCI_Vw_no_penalty/"
...
Ci[:,:,:] = np.where(Ci[:,:,:] < 0, 0, Ci[:,:,:])

ntime, nlat, nlon = np.shape(Ci)

Ci_diff = np.zeros((3, nlat, nlon), dtype = np.float32)
...
nco = Dataset("/esd/esd01/docs/hsharma/data_small/FCI_data/FCI_Vw_no_penalty/FCI_Vw_diff.nc",'w', format="NETCDF4_CLASSIC")
...
timeo[:] = [1,2,3]			# 1 -> PI-MH, 2 -> PI-LGM, 3 -> PI-PLIO
lato[:] = nc1.variables['lat'][:]
lono[:] = nc1.variables['lon'][:]
Ci_o[:,:,:] = Ci_diff[:,:,:]
nco.close()

Ci_diff[0,:,:] = np.where(Ci_diff[0,:,:] < 0, 0, Ci_diff[0,:,:])
Ci_diff[1,:,:] = np.where(Ci_diff[1,:,:] < 0, 0, Ci_diff[1,:,:])
Ci_diff[2,:,:] = np.where(Ci_diff[2,:,:] < 0, 0, Ci_diff[2,:,:])

print('PI_MH min: ', np.min(Ci_diff[0,:,:]))
print('PI_MH max: ', np.max(Ci_diff[0,:,:]))

print('PI_LGM min: ', np.min(Ci_diff[1,:,:]))
print('PI_LGM max: ', np.max(Ci_diff[1,:,:]))

print('PI_PLIO min: ', np.min(Ci_diff[2,:,:]))
print('PI_PLIO max: ', np.max(Ci_diff[2,:,:]))
