...
file1 = "1003_1017_Plio.nc"            			# File containing MAT and Ta for Pliocene simulation
...
S = "PLIO"						# Defining the time-slice for the current file
...
nc_SLM = Dataset('/esd/esd02/data/climate_models/echam/echam_output/ESD/e010_hpc-bw_e5w2.3_PLIO_t159l31.1d/src/input/T159_PLIO/T159_jan_surf_plio.nc')
nc_GLAC = Dataset('/esd/esd02/data/climate_models/echam/echam_output/ESD/e009_hpc-bw_e5w2.3_LGM_t159l31.1d/src/input/T159_LGM/T159_jan_surf_lgm_veg.nc')
SLM = nc_SLM.variables['SLM'][:,:]
GLAC = nc_GLAC.variables['GLAC'][:,:]
...
for i in range(nlat):
	for j in range(nlon):
		arg.append((MAT[i,j], Ta[i,j], Hs[i,j], SLM[i,j], GLAC[i,j]))		
...
############################################################################
# Write out data to a new netCDF file for the current time slice (Pliocene)
############################################################################
    
nco = Dataset("/esd/esd01/docs/hsharma/data_small/FCI_data/FCI_Vw_no_penalty/Integrated_FCI_%s_Vw.nc" % (S,),'w', format="NETCDF4_CLASSIC")
